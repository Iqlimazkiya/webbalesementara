...
<section class="relative min-h-screen flex items-center px-8 md:px-20 lg:px-28 overflow-hidden bg-[#0a0f0a]">
    ...
    @if(!empty($home->hero_video) && $heroIsImage)
    <img src="{{ asset('storage/' . $home->hero_video) }}" 
         class="absolute inset-0 w-full h-full object-cover z-0" alt="Hero">
    @elseif(!empty($home->hero_video))
    <video autoplay loop muted playsinline class="absolute inset-0 w-full h-full object-cover z-0">
        <source src="{{ asset('storage/' . $home->hero_video) }}" type="video/mp4">
    </video>
    @else
    <video autoplay loop muted playsinline poster="{{ asset('assets/images/home-poster.jpg') }}" class="absolute inset-0 w-full h-full object-cover z-0">
...
<div class="lg:col-span-7 space-y-6">
            <div class="space-y-2">
                <p class="animate-fadeUp text-gray-400 tracking-[0.3em] uppercase text-[15px] font-medium">
                    {{ $home->hero_teks_baris1 ?? 'Welcome To' }}
                </p>
                <h1 class="animate-fadeUp animation-delay-200 font-serif leading-tight tracking-tight">
                    <span class="text-[#c5a059] text-4xl md:text-5xl lg:text-6xl block">{{ $home->hero_teks_baris2 ?? 'Bale Hinggil' }}</span>
                    <span class="text-white text-3xl md:text-4xl lg:text-5xl font-light block italic">{{ $home->hero_teks_baris3 ?? 'Apartment' }}</span>
                </h1>
            </div>
            ...
            <p class="animate-fadeUp animation-delay-600 text-gray-300 text-sm md:text-base max-w-xl leading-relaxed font-light">
                {!! nl2br(e($home->hero_subjudul ?? 'Apartemen 31 lantai yang dirancang dengan konsep adequate apartment.')) !!}
            </p>
            <div class="animate-fadeUp animation-delay-800 flex flex-row gap-3 pt-6 w-full max-w-xs sm:max-w-sm md:max-w-none">
    <a href="{{ $home->hero_btn1_link ?? route('user.tipeunit') }}"
       class="hero-btn-glow flex-1 text-center bg-[#c5a059] text-black px-4 py-3 md:px-6 md:py-4 font-semibold uppercase text-[11px] md:text-sm flex items-center justify-center gap-2 tracking-wider md:tracking-widest rounded-xl transition-all duration-300 hover:bg-[#d4af6a] hover:scale-[1.02] hover:-translate-y-0.5 hover:shadow-[0_0_12px_rgba(197,160,89,0.5)] relative overflow-hidden">
        {{ $home->hero_btn1_teks ?? 'Jelajahi Unit' }} <span>→</span>
    </a>
    @php
    ...
    @endphp
       ...
       class="hero-btn-glow flex-1 text-center border-2 border-[#c5a059] text-[#c5a059] rounded-xl px-4 py-3 md:px-6 md:py-4 font-semibold uppercase text-[11px] md:text-sm tracking-wider md:tracking-widest transition-all duration-300 flex items-center justify-center hover:bg-[#c5a059] hover:text-black hover:scale-[1.02] hover:-translate-y-0.5 hover:shadow-[0_0_12px_rgba(197,160,89,0.45)] relative overflow-hidden">
        {{ $home->hero_btn2_teks ?? 'Hubungi Kami' }}
    </a>
</div>
{{-- Mobile cards: hanya tampil di bawah lg --}}
    ...
    <div class="max-w-7xl mx-auto">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start mb-12">
            <div class="lg:col-span-5 relative fade-up-scroll flex flex-col gap-6">
                @if(!empty($home->about_foto))
                <img id="about-foto-img" src="{{ asset('storage/' . $home->about_foto) }}" alt="About"
                     class="rounded-[0.5rem] w-full aspect-[3/3] object-cover border border-[#c5a059]/20 shadow-2xl">
                @else
                <img id="about-foto-img" src="{{ asset('assets/img/rating.jpeg') }}" alt="About"
                     class="rounded-[0.5rem] w-full aspect-[3/3] object-cover border border-[#c5a059]/20 shadow-2xl">
                @endif
                ...
                <div class="bg-[#1a3d2a] p-6 rounded-md border border-[#c5a059]/20 flex items-center justify-between shadow-xl">
                    <div class="flex items-center gap-4">
                        <span class="text-4xl font-serif text-white">{{ $home->about_review_rating ?? '3,6' }}</span>
                        <div class="flex flex-col">
                            ...
                            <p class="text-[10px] text-gray-400 uppercase tracking-widest leading-none mt-1">
                                {{ $home->about_review_jumlah ?? '383 Reviews on Google' }}
                            </p>
                        </div>
                    </div>
                    <a href="{{ $home->about_review_link ?? '#' }}" target="_blank"
        ...
        <div class="fade-up-scroll grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
            ...
            <div class="flex flex-col gap-6">
                ...
                <div class="bg-[#1a3d2a]/60 border border-[#c5a059]/15 rounded-2xl p-8">
                    ...
                    <div class="space-y-4">
                        @foreach(['Menjadi developer properti terpercaya yang menghadirkan hunian berkualitas, bernilai investasi tinggi, dan berkontribusi terhadap pertumbuhan kawasan modern di Indonesia.'] as $visi)
                        <p class="text-gray-400 text-sm leading-relaxed">{{ $visi }}</p>
                        @endforeach
                    </div>
                </div>
                ...
                <div class="bg-[#1a3d2a]/60 border border-[#c5a059]/15 rounded-2xl p-8">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="flex items-start gap-3">
                            <span class="mt-0.5 w-5 h-5 rounded-md bg-[#0f2a1f] border border-[#c5a059]/20 flex items-center justify-center shrink-0 text-[#c5a059] text-[10px] font-bold">{{ $i + 1 }}</span>
                            <p class="text-gray-400 text-sm leading-relaxed">{{ $misi }}</p>
                        </div>
                        @endforeach
                    </div>
                </div>
...
<section id="layanan" class="bg-[#fff6e5] text-[#0f2a1f]">
    ...
    <div class="relative">
        <div class="h-auto min-h-[480px] bg-cover bg-center relative"
            style="background-image: linear-gradient(rgba(0,0,0,0.45), rgba(0,0,0,0.55)), url('{{ !empty($home->layanan_foto_bg) ? asset('storage/' . $home->layanan_foto_bg) : asset('assets/img/layanan.jpg') }}');">
            ...
            <div class="absolute inset-0 flex items-center justify-center px-8 md:px-20 py-14">
                <div class="fade-up-scroll grid grid-cols-2 md:grid-cols-3 w-full max-w-3xl" style="gap: 10px 28px;">
                   ...
                   @foreach($layananBtns as $index => $btn)
<a href="{{ $btn['teks'] === 'Commercial Area' ? route('commercial-area') : ($btn['link'] ?? '#') }}"
   ...
   ">
    ...
    <div class="flex items-center gap-3 relative z-10">
        <span class="layanan-icon flex items-center justify-center w-9 h-9 rounded-xl flex-shrink-0 transition-all duration-300"
            ...
            {!! $layananSvgIcons[$index % count($layananSvgIcons)] !!}
        </span>
...
<section id="fasilitas" class="relative bg-[#0f2a1f] pt-10 pb-12 scroll-mt-70">
    <div class="max-w-7xl mx-auto px-8 md:px-20 lg:px-28">
        <div class="fade-up-scroll text-center mb-16">
            <p class="text-sm tracking-[4px] text-[#c6a85b] uppercase mb-4">{{ $home->fasilitas_tag ?? 'Fasilitas' }}</p>
            <h2 class="text-white text-4xl md:text-5xl lg:text-6xl font-serif font-light leading-tight">
                {{ $home->fasilitas_judul1 ?? 'Hidup Lebih dari' }}
                <span class="italic text-[#c6a85b]">{{ $home->fasilitas_judul2 ?? 'Sekadar Hunian' }}</span>
            </h2>
            <div class="w-20 h-[2px] bg-[#c6a85b] mx-auto my-6"></div>
            <p class="text-white max-w-2xl mx-auto text-lg leading-relaxed">
                {{ $home->fasilitas_deskripsi ?? '' }}
            </p>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            @php $firstItem = $fasItems[0] ?? null; @endphp
            @if($firstItem)
...
@endphp
                <img src="{{ $firstFotoUrl }}" class="w-full h-[235px] lg:h-[500px] object-cover transition duration-700 group-hover:scale-110">
                <div class="absolute inset-0 bg-gradient-to-t from-black/70 via-black/30 to-transparent"></div>
                <div class="absolute bottom-8 left-8 text-white">
                    <h3 class="text-3xl font-serif mb-2">{{ $firstItem['judul'] ?? '' }}</h3>
                    <p class="text-gray-200 max-w-md">{{ $firstItem['keterangan'] ?? '' }}</p>
                </div>
            </div>
            @endif
            ...
            @endphp
            <div class="flex flex-col gap-6">
                @foreach(array_slice($fasItems, 1) as $fasIdx => $fas)
                <div class="fade-up-scroll relative group overflow-hidden rounded-2xl">
                    ...
                    @endphp
                    <img src="{{ $fasFotoUrl }}" class="w-full h-[235px] object-cover transition duration-700 group-hover:scale-110">
                    <div class="absolute inset-0 bg-gradient-to-t from-black/70 via-black/30 to-transparent"></div>
                    <div class="absolute bottom-6 left-6 text-white">
                        <h3 class="text-xl font-serif mb-1">{{ $fas['judul'] ?? '' }}</h3>
                        <p class="text-gray-200 text-sm">{{ $fas['keterangan'] ?? '' }}</p>
                    </div>
                </div>
                @endforeach
            </div>
...
<section id="lokasi" class="py-12 bg-[#fff6e5] font-sans">
    <div class="max-w-7xl mx-auto px-8 md:px-20 lg:px-28">
        <div class="text-center mb-10 fade-up-scroll">
            <div class="flex items-center justify-center gap-3 mb-2">
                ...
                <span class="text-[#c5a059] font-bold tracking-[0.2em] uppercase text-xs">{{ $home->lokasi_tag ?? 'Lokasi' }}</span>
                <div class="h-[1px] w-10 bg-[#c5a059]"></div>
            </div>
            <h2 class="text-3xl md:text-5xl font-serif text-[#0f2a1f] leading-tight">
                {{ $home->lokasi_judul1 ?? 'Strategis di Jantung' }}<br>
                <span class="italic text-[#c5a059]">{{ $home->lokasi_judul2 ?? 'Surabaya Timur' }}</span>
            </h2>
        </div>
    ...
    @if(!empty($home->lokasi_gmaps_embed))
        {!! $home->lokasi_gmaps_embed !!}
    @else
    ...
    @endif
            ...
            <div class="flex flex-col gap-5">
                ...
                <div class="fade-up-scroll bg-[#0f2a1f] p-8 rounded-[1rem] shadow-lg">
                    ...
                    <h3 class="text-white text-2xl md:text-3xl font-bold mb-2">{{ $home->lokasi_nama_gedung ?? 'Apartement Bale Hinggil' }}</h3>
                    <p class="text-gray-400 text-xs leading-relaxed">{{ $home->lokasi_alamat_lengkap ?? '' }}</p>
                </div>
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="flex justify-between items-center text-xs group">
                            <div class="flex items-center gap-3">
                                <span class="w-7 h-7 flex items-center justify-center bg-[#0f2a1f] rounded text-sm flex-shrink-0">{{ $aksesIcons[$ai] ?? '📍' }}</span>
                                <span class="text-white group-hover:text-[#c5a059] transition">{{ $akses['nama'] ?? '' }}</span>
                            </div>
                            <span class="text-[#c5a059] font-medium ml-3 flex-shrink-0">{{ $akses['waktu'] ?? '' }}</span>
                        </div>
                        @if(!$loop->last)<div class="h-[1px] w-full bg-white/5"></div>@endif
                        @endforeach
                    </div>
                </div>
                @endif
        ...
        <div class="flex flex-col md:flex-row md:items-end md:justify-between mb-12 gap-4">
            <div>
                <p class="fade-left-scroll text-[#c5a059] font-medium tracking-[0.3em] uppercase text-sm mb-4">
                    {{ $home->berita_tag ?? 'BERITA' }}
                </p>
                <h2 class="fade-left-scroll font-serif leading-[1.1] text-gray-900">
                    <span class="block text-4xl md:text-5xl">{{ $home->berita_judul1 ?? 'Update Terkini' }}</span>
                    <span class="block text-[#c5a059] italic text-3xl md:text-4xl mt-2">{{ $home->berita_judul2 ?? 'Bale Hinggil' }}</span>
                </h2>
            </div>
            @if(!empty($home->berita_link_semua))
            <a href="{{ $home->berita_link_semua }}" target="_blank"
        ...
        <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            <div class="fade-up-scroll bg-[#0f2a1f] rounded-2xl overflow-hidden shadow-lg hover:-translate-y-2 hover:shadow-2xl transition duration-300 group flex flex-col h-full">
                ...
                <div class="p-6 text-white flex-1 flex flex-col">
                    <p class="text-sm text-gray-300 mb-2">{{ $yt['tanggal'] ?? '' }}{{ !empty($yt['tanggal']) && !empty($yt['durasi']) ? ' • ' : '' }}{{ $yt['durasi'] ?? '' }}</p>
                    <h3 class="text-xl font-semibold leading-snug mb-3">{{ $yt['judul'] ?? '' }}</h3>
                    <p class="text-gray-300 text-sm mb-6">{{ Str::limit($yt['deskripsi'] ?? '', 100) }}</p>
                    <div class="mt-auto flex items-center justify-between text-sm">
                        <span class="text-gray-400">{{ $yt['channel'] ?? '' }}</span>
                        <a href="{{ $ytLink }}" target="_blank" class="text-[#c5a059] font-semibold hover:text-white transition-all duration-300">Tonton →</a>
                    </div>
                </div>
            </div>
            @endforeach

            {{-- 3 Card Berita / Artikel --}}
            @foreach($artikelItems as $art)
            <div class="fade-up-scroll bg-[#0f2a1f] rounded-2xl overflow-hidden shadow-lg hover:-translate-y-2 hover:shadow-2xl transition duration-300 flex flex-col h-full">
                <div class="relative">
                    ...
                    @endphp
                    <img src="{{ $artFotoUrl }}" class="w-full h-56 object-cover" />
                    <span class="absolute top-4 left-4 bg-[#c5a059] text-white text-xs font-bold px-4 py-1 rounded-full">BERITA</span>
                </div>
                <div class="p-6 text-white flex-1 flex flex-col">
                    <p class="text-sm text-gray-300 mb-2">{{ $art['tanggal'] ?? '' }}{{ !empty($art['tanggal']) && !empty($art['jam']) ? ' • ' : '' }}{{ $art['jam'] ?? '' }}</p>
                    <h3 class="text-xl font-semibold mb-3">{{ $art['judul'] ?? '' }}</h3>
                    <p class="text-gray-300 text-sm mb-6">{{ Str::limit($art['ringkasan'] ?? '', 120) }}</p>
                    <div class="mt-auto flex items-center justify-between text-sm">
                        <span class="text-gray-400">{{ $art['penerbit'] ?? '' }}</span>
                        @if(!empty($art['link']))
                        <a href="{{ $art['link'] }}" target="_blank" class="text-[#c5a059] font-semibold hover:text-white transition-all duration-300">Baca →</a>
                        @endif
                    </div>
                </div>
            </div>
...
<section id="kontak" class="bg-[#fff6e5] pb-0 px-0 scroll-mt-78">
    ...
    <div>
        <h2 class="text-2xl font-serif mb-2">{{ $home->kontak_judul ?? 'Informasi Kontak' }}</h2>
                    <div class="w-12 h-[1px] bg-[#c5a059] mb-10"></div>
                    <div class="space-y-8">
                        
                        @if(!empty($home->kontak_telepon))
                        <div class="flex items-start gap-4">
                            ...
                            <div>
                                ...
                                <p class="text-lg font-light">{{ $home->kontak_telepon }}</p>
                            </div>
                        </div>
                        @endif
                        ...
                        <div class="flex items-start gap-4">
                            ...
                            <div>
                                ...
                                <p class="text-base font-light leading-relaxed">
                                    @foreach($jamList as $jam){{ $jam }}<br>@endforeach
                                </p>
                            </div>
                        </div>
                        @endif
                ...
                <div class="mt-12 lg:mt-0">
                    ...
                    <div class="flex gap-4">
                        ...
                        @if(!empty($s['icon']))
                        <a href="{{ !empty($s['link']) ? $s['link'] : '#' }}"
                           {{ !empty($s['link']) ? 'target=_blank' : '' }}
                           class="w-10 h-10 rounded-xl border border-white/10 flex items-center justify-center hover:border-[#c5a059] transition group">
                            {!! $sosmedSvgs[$s['icon']] ?? '<svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-[#c5a059]" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101" /></svg>' !!}
                        </a>
                        @endif
                        @endforeach
                    </div>
            ...
            <div class="lg:col-span-8 bg-[#132b21] px-8 md:px-14 lg:px-20 pr-8 md:pr-24 lg:pr-28 py-10 md:py-14 border-l border-white/5">
                @if(session('contact_success') || session('success'))
                ...
                <form action="{{ route('home.store') }}" method="POST" class="space-y-6">
                    @csrf
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="divisi" class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-4 text-white focus:outline-none focus:border-[#c5a059] transition appearance-none">
                            ...
                            <option value="{{ $div }}" class="bg-[#132b21]">{{ $div }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="space-y-2">
...
@if(session('contact_success') || session('success') || $errors->any())
<script>
document.addEventListener('DOMContentLoaded', function () {
